...
Sorguyu dinamiklestirebildigimiz icin tercih edilir.
*/
...
-- Honda sirketinin calisan sayisini 16000 olarak guncelleyiniz.sirketler
UPDATE sirketler
...
/*----------------------------------------------------------------                
  3) Ankara'daki calisani olan sirketlerin sirket id ve personel 
  sayilarini listeleyiniz.
----------------------------------------------------------------*/

SELECT sirket_id, calisan_sayisi
FROM sirketler
WHERE sirket IN(SELECT sirket FROM atom_karincalar WHERE sehir  ='Ankara');
...
FROM sirketler
WHERE sirket IN(SELECT sirket FROM atom_karincalar WHERE isim = 'Veli Yilmaz');
...
/*----------------------------------------------------------------
 ...
 ortalama maas yok, onu biz olusturacagiz
----------------------------------------------------------------*/
...
SELECT sirket_id, sirket, (SELECT COUNT(distinct sehir) 
							FROM atom_karincalar 
							WHERE atom_karincalar.sirket=sirketler.sirket)sehirler
FROM sirketler;
...
SELECT sirket, calisan_sayisi, (SELECT MAX(maas) 
							...
							WHERE atom_karincalar.sirket = sirketler.sirket)min_maas,
                             (SELECT AVG(maas) 
							FROM atom_karincalar 
							WHERE atom_karincalar.sirket = sirketler.sirket)avg_maas
FROM sirketler;


/*----------------------------------------------------------------
SORU 6- Ismi 'H' ile baslayan sirketlerin ismini, personel sayisini ve iscilere
