...
SELECT isim, SUM(urun_miktar) toplam_kilo
FROM manav
GROUP BY isim
ORDER BY isim;

-- En cok alanlara gore listersek

SELECT isim,SUM(urun_miktar) AS top_kg
FROM manav
GROUP BY isim
ORDER BY top_kg DESC, isim;
...
SELECT urun_adi, MIN(urun_miktar)minMiktar, MAX(urun_miktar)maxMiktar
FROM manav
WHERE urun_adi IS NOT NULL
GROUP BY urun_adi
ORDER BY maxMiktar ;
...
SELECT isim, urun_adi, SUM(urun_miktar)toplamUrun
FROM manav
GROUP BY isim, urun_adi
ORDER BY isim desc, urun_adi; -- Burada isim ve urun adini birlikte bir butun olarak algiladigi icin 
									-- siralamada urun adini da goz onunde bulunduruyor. 
                    
                    
SELECT urun_adi, isim, SUM(urun_miktar)toplamUrun
FROM manav
