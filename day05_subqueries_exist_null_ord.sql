...
urunleri mayis ayinda alanlarin/ isimlerini getiren bir query yaziniz.
----------------------------------------------------------------*/ 
...
FROM nisan_satislar
WHERE EXISTS (SELECT urun_isim FROM mayis_satislar WHERE nisan_satislar.urun_isim=mayis_satislar.urun_isim);
...
SELECT urun_isim, musteri_isim
FROM nisan_satislar
WHERE NOT EXISTS (SELECT urun_isim FROM mayis_satislar WHERE nisan_satislar.urun_isim=mayis_satislar.urun_isim);
...
SELECT urun_isim, musteri_isim
FROM mayis_satislar
WHERE NOT EXISTS (SELECT urun_isim FROM nisan_satislar WHERE mayis_satislar.urun_isim=nisan_satislar.urun_isim);
...
=> !!!!! ___ Ayni degerde olan kayitlari, tablodaki kayit sirasina gore getirir.___!!!!!!!
==============================================================*/
...
SELECT *
FROM insanciklar
ORDER BY ssn ASC; -- ayni olan degerleri tabloda ki kayit sirasina gore listeliyor
...
SELECT *
FROM insanciklar
ORDER BY soyisim DESC, isim;  -- ilk kosulda ayni degerler varsa,(kayit sirasinin aksine) bu ayni olan degerleri ikinci kosuldaki doneye gore siralama yapar.
...
SELECT *
FROM insanciklar
ORDER BY adres DESC, isim DESC, soyisim DESC; -- once adrese gore, sonra esitlik varsa isme gore, yine esitlik varsa 3. kistasa gore siralayacak. Sart yoksa naturel order a gore siralar.
